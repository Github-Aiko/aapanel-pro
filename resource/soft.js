var soft = {
    ...
    set_soft_tips: function(rdata, type) {
        ...
        if (type == 10) {
            ...
            btn_ground = soft.render_tips_btn(btn_ground, [
                { title: 'Zalo admin', rel: 'noreferrer noopener', href: 'https://t.me/Tele_Aiko', target: '_blank', btn: 'Zalo admin', class: 'btn btn-success btn-xs va0 ml15', style: "margin-left:10px;" },
                {
            ...
            if ((ltd > 0 && (ltd == pro || pro < 0)) || (ltd < 0 && pro >= 0) || (ltd > 0 && pro >= 0)) {
                ...
                explain.html('bản ' + (_ltd ? 'Pro' : 'Pro') + ' một ' + (_ltd ? '专业版及企业版插件' : 'phiên bản giới hạn,') + (!(pro == 0 && ltd < 0) ? ('expiration time: ' + (bt.format_data((_ltd ? ltd : pro), 'yyyy/MM/dd')) + '' + ((((_ltd ? ltd : pro) - todayDate) <= 15 * 24 * 60 * 60) ? ('，<span style="color:red">Only ' + Math.round(((_ltd ? ltd : pro) - todayDate) / (24 * 60 * 60)) + ' days until expiration</span>') : '')) : 'thời hạn sử dụng：<span style="color: #fc6d26;font-weight: bold;">Không giới hạn</span>'));
            } else if (ltd == -1 && pro == -1) {
                explain.html('Upgrade to Pro edition, all plugins, free to use!');
            } else if (pro == 0 && ltd < 0) {
                _ltd = 2;
                explain.html('AikoCute' + (type == 12 ? '&nbsp;&nbsp;<span style="color:#af8e48">Sau khi nâng cấp Phiên bản Doanh nghiệp, doanh nghiệp có thể dùng thử miễn phí trình cắm Phiên bản Doanh nghiệp và Trình cắm Phiên bản Chuyên nghiệp.</span>' : ''));
                if (type == 12) {
            ...
            var set_btn_style = function(res) {
                ...
                if (_this.trail) {
                    setTimeout(function() {
                        $('.btn-ground').after('<span class="pro_trail" style="font-weight: 700;margin-left:25px;">Liên hệ Aiko</span>')
                        var trail = $('<a href=\"https://t.me/Tele_Aiko\" style="margin-left:10px;">ấn vào đây</a>');
    ...
    get_tab_contents: function(key, obj) {
